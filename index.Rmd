...
```{r}
vac <- dbGetQuery(db,"select * from OWID where location = 'Pakistan'")
...
#   labs(title="Pakistan's Daily Doses By Date") 
...
#   scale_y_continuous(labels=comma) +
#   labs(title="Pakistan's Total (Full) Doses Date") 
# ggplotly(p6)
```
...
#   scale_y_continuous(labels=comma) +
#   labs(title="Pakistan's Number of People Vaccinatations By Date") 
...
<!-- p6 <- ggplot(vac) + geom_line(aes(x=date,y=total_vaccinations)) + -->
<!--   scale_y_continuous(labels=comma) + -->
<!--   labs(title="Pakistan's Total  Number of People Fully Vaccinated By Date")  -->
<!-- ggplotly(p6) -->
